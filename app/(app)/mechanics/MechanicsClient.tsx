...
export default function MechanicsClient({
  ...
  function upsert(updated: Mechanic) {
    setMechanics((prev) => {
  ...
  async function addMechanic(name: string): Promise<string | null> {
    try {
  ...
  async function renameMechanic(id: number, name: string): Promise<string | null> {
    try {
  ...
  async function toggleActive(mechanic: Mechanic) {
    ...
    try {
      ...
      const json = await res.json()
      if (res.ok) upsert(json.mechanic)
    } finally {
      setToggling(null)
    }
  }
